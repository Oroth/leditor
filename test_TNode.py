...
from TNode import TNode, createTreeFromSexp, copyTNode, Cursor, replaceAdd, deleteAdd, insertAdd
...
class TestCursor(TestCase):
    ...
    def test_next(self):
        c = Cursor(self.tree, [0]).next()
        self.assertEqual(c.get().child, 4)

    def test_next2(self):
        c = Cursor(self.tree, [0]).next().next()
    ...
    def test_prev2(self):
        c = Cursor(self.tree, [3]).prev().prev()
        self.assertEqual(c.get().child, 4)

    def test_prev3(self):
        c = Cursor(self.tree, [3, 1]).prev()
